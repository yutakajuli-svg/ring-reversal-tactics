...
export default function Home() {
  ...
  const [facingSet, setFacingSet] = useState(false);
  ...
  const frontPos = front(player, playerFacing);
  const frontKey = inside(frontPos) ? `${frontPos.r},${frontPos.c}` : '';
  ...
  function movePlayer(destination: Pos) {
    ...
    setPlayer(destination); setPlayerFacing(faceToward(previous, destination)); setFacingSet(false); setMoved(true);
    setMessage('移動完了。次に矢印を1つ選び、向きを確定してください。');
  }

  function finishRound(nextPlayer = player, cancelled = false, reserved = reservedStrike, baseHits = hits) {
    if (cancelled) {
      setMoved(false); setFacingSet(false); setReservedStrike(null); setRound((n) => n + 1);
    ...
    setIntent(makeCpuIntent(nextCpu, nextPlayer)); setMoved(false); setFacingSet(false); setReservedStrike(null); setRound((n) => n + 1);
  ...
  function reset() {
    ...
    setRound(1); setFacingSet(false); setReservedStrike(null); setWinner(null); setMessage('CPUの予定を見て、移動先を選んでください。');
  ...
  return (
    <main className="game-shell">
      ...
      <section className="score-row" aria-label="試合状況">
        <div className="fighter-card player-card"><div><span>PLAYER</span><strong>RED CORNER</strong></div><div className="hit-pips" aria-label={`成功打撃 ${hits.player}/3`}>{[0,1,2].map((n)=><i key={n} className={n<hits.player?'on':''}/>)}</div></div>
        <div className="versus">FIRST TO 3</div>
        <div className="fighter-card cpu-card"><div><span>CPU</span><strong>BLUE CORNER</strong></div><div className="hit-pips" aria-label={`成功打撃 ${hits.cpu}/3`}>{[0,1,2].map((n)=><i key={n} className={n<hits.cpu?'on':''}/>)}</div></div>
      </section>
      <div className="game-grid">
        ...
        <section className="ring-wrap" aria-label="7×7リング"><div className="ringside-floor" aria-hidden="true"/><div className="board">
          <i className="ring-wall wall-left" aria-hidden="true"/><i className="ring-wall wall-right" aria-hidden="true"/>
          <i className="corner-block corner-top" aria-hidden="true"/><i className="corner-block corner-right" aria-hidden="true"/><i className="corner-block corner-bottom" aria-hidden="true"/><i className="corner-block corner-left" aria-hidden="true"/>
        ...
        </div><div className="ring-label">7 × 7 RING</div></section>
